<!DOCTYPE html>
<!-- Template Name: Packet - Responsive Admin Template build with Twitter Bootstrap 3.x | Author: ClipTheme -->
<!--[if IE 8]><html class="ie8" lang="en"><![endif]-->
<!--[if IE 9]><html class="ie9" lang="en"><![endif]-->
<!--[if !IE]><!-->
<html lang="en">
<!--<![endif]-->
<!-- start: HEAD -->
<head>
	<title>Packet - Responsive Admin Template</title>
	<?php include 'include/main-head.php'; ?>
	<!-- Favicon -->
	<link rel="shortcut icon" href="favicon.ico" />
	<link rel="stylesheet" href="assets/css/rio.css">
</head>
<!-- end: HEAD -->
<body>
<div id="app" class="lyt-2">
	<!-- sidebar -->
	<?php include 'include/sidebar.php'; ?>
	<!-- / sidebar -->
	<div class="app-content">
		<!-- start: TOP NAVBAR -->
		<?php include 'include/nav-header.php'; ?>
		<!-- end: TOP NAVBAR -->
		<div class="main-content" >
			<div class="wrap-content container" id="container">
				<!-- start: BREADCRUMB -->
				<div class="breadcrumb-wrapper">
					<a href="index.php"><h4 class="mainTitle no-margin">Dashboard</h4></a>
					<h5>Welcome to Beon Accounting</h5>
				</div>
				<!-- end: BREADCRUMB -->
				<!-- start: FIRST SECTION -->
				<div class="container padding-left-30 padding-right-30 padding-top-30">
					<div class="row">
						<div class="col-sm-3 col-xs-3">
							<h5 class="over-title margin-bottom">Laporan <span class="text-bold">Mutasi Keuangan</span></h5>
						</div>
						<div class="col-sm-3 col-xs-0"></div>
						<div class="pull-right">
							Sort By
							<div class="btn-group">
								<button type="button" class="btn btn-light-grey">
									Tanggal Input
								</button>
								<button type="button" class="btn btn-light-grey dropdown-toggle" data-toggle="dropdown" aria-expanded="true">
									<span class="fa fa-angle-down"></span>
								</button>
								<ul role="menu" class="dropdown-menu dropdown-light pull-right">
									<li>
										<a href="#"> Action </a>
									</li>
									<li>
										<a href="#"> Another action </a>
									</li>
									<li>
										<a href="#"> Something else here </a>
									</li>
								</ul>
							</div>
						</div>
						<div class="col-sm-3 col-xs-12 pull-right margin-bottom-20">
							<span class="input-icon input-icon-right">
								<input type="text" placeholder="Search..." id="form-field-17" class="form-control">
								<i class="fa fa-search"></i>
							</span>
						</div>
					</div>
					<div class="row margin-bottom">
						<div class="panel panel-white">
							<div class="panel-body">
								<div class="table">
									<div class="pull-right margin-bottom-5">
										<a class="btn btn-primary btn-o btn-wide tombol">Export </a>
										<i class="fa fa-calendar padding-left-20"></i> 11 November 2015
									</div>
									<table class="table table-bordered" id="sample-table-1">
										<thead>
											<tr class="paddings odd">
												<th class="paddings text-center tabelmut">Bulan Lalu</th>
												<th colspan="2" class="paddings text-center">Perkiraan</th>
												<th class="paddings text-center">Bulan Ini</th>
											</tr>
										</thead>
										<tbody>
											<tr>
												<td class="paddings tabelmut"></td>
												<td width="4%" class="text-center text-bold">I. </td>
												<td class="paddings text-bold">Saldo Awal</td>
												<td class="paddings tabelmut"></td>
												</tr>
											<tr>
												<td class="text-right padding-right-5">65.324.944,00</td>
												<td></td>
												<td class="paddings">Tunai : (Rp)</td>
												<td class="text-right padding-right-5">22.828.246</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">-</td>
												<td></td>
												<td class="paddings">Ch / GB diterima</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">(0,39)</td>
												<td></td>
												<td class="paddings">Bank Niaga</td>
												<td class="text-right padding-right-5">(0)</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">0,06</td>
												<td></td>
												<td class="paddings">Bank BRI Syariah</td>
												<td class="text-right padding-right-5">(0)</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">290.638,62</td>
												<td></td>
												<td class="paddings">Bank Bukopin Syariah</td>
												<td class="text-right padding-right-5">290.639</td>
											</tr>
											<tr>
												<td class="padding-right-5 text-right text-bold">415.311.505,21</td>
												<td></td>
												<td class="paddings text-center text-bold">Sub Jumlah</td>
												<td class="padding-right-5 text-right text-bold">585.554.803</td>
											</tr>
											<tr>
												<td></td>
												<td></td>
												<td></td>
												<td></td>
											</tr>
											<tr>
												<td class="text-right padding-right-5"></td>
												<td class="text-center text-bold">II. </td>
												<td class="paddings text-bold">Penerimaan</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">-</td>
												<td class="text-center">**</td>
												<td class="paddings">Penyesuaian saldo debet bank</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">-</td>
												<td class="text-center">**</td>
												<td class="paddings">Piutang penjualan</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">-</td>
												<td class="text-center">**</td>
												<td class="paddings">Piutang direksi</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">-</td>
												<td class="text-center">**</td>
												<td class="paddings">Piutang pihak III</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">2.400.000,00</td>
												<td class="text-center">**</td>
												<td class="paddings">Piutang karyawan</td>
												<td class="text-right padding-right-5">1.000.000</td>
											</tr>
											<tr>
												<td class="text-right text-bold padding-right-5">1.987.348.960,58</td>
												<td></td>
												<td class="paddings text-bold">Sub Jumlah</td>
												<td class="text-right text-bold padding-right-5">-</td>
											</tr>
											<tr>
												<td></td>
												<td></td>
												<td></td>
												<td></td>
											</tr>
											<tr>
												<td class="text-right padding-right-5"></td>
												<td class="text-center text-bold">III. </td>
												<td class="paddings text-bold">Pengeluaran</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">-</td>
												<td class="text-center">**</td>
												<td class="paddings">Penyesuaian saldo kredit bank</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">-</td>
												<td class="text-center">**</td>
												<td class="paddings">Setoran dalam perjalanan</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">-</td>
												<td class="text-center">**</td>
												<td class="paddings">Piutang direksi</td>
												<td class="text-right padding-right-5">12.149.176</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">-</td>
												<td class="text-center">**</td>
												<td class="paddings">Piutang pihak ketiga</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">7.000.000,00</td>
												<td class="text-center">**</td>
												<td class="paddings">Piutang karyawan</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="padding-right-5 text-right text-bold">1.817.105.662,97</td>
												<td></td>
												<td class="paddings text-center text-bold">Sub Jumlah</td>
												<td class="padding-right-5 text-right text-bold">724.772.960</td>
											</tr>
											<tr>
												<td></td>
												<td></td>
												<td></td>
												<td></td>
											</tr>
											<tr>
												<td class="paddings"></td>
												<td width="2%" class="text-center text-bold">IV. </td>
												<td class="paddings text-bold">Saldo Akhir</td>
												<td class="paddings"></td>
												</tr>
											<tr>
											<tr>
												<td class="text-right padding-right-5">22.828.246,00</td>
												<td class="text-center"></td>
												<td class="paddings">Tunai : (Rp)</td>
												<td class="text-right padding-right-5">27.716.936</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">-</td>
												<td class="text-center"></td>
												<td class="paddings">CH / GB diterima</td>
												<td class="text-right padding-right-5"></td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">(0,39)</td>
												<td class="text-center"></td>
												<td class="paddings">Bank Niaga</td>
												<td class="text-right padding-right-5">(0)</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">(0,06)</td>
												<td class="text-center"></td>
												<td class="paddings">Bank BRI Syariah</td>
												<td class="text-right padding-right-5">(0)</td>
											</tr>
											<tr>
												<td class="text-right padding-right-5">290.638,62</td>
												<td class="text-center"></td>
												<td class="paddings">Bank Bukopin Syariah</td>
												<td class="text-right padding-right-5">-</td>
											</tr>
											<tr>
												<td class="padding-right-5 text-right text-bold">585.554.802,82</td>
												<td></td>
												<td class="paddings text-center text-bold">Jumlah</td>
												<td class="padding-right-5 text-right text-bold">704.047.756</td>
											</tr>
										</tbody>
									</table>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
	<!-- start: FOOTER -->
	<?php include 'include/footer.php'; ?>
	<!-- end: FOOTER -->
	<!-- start: OFF-SIDEBAR -->
	<?php include 'include/off-sidebar.php'; ?>
	<!-- end: OFF-SIDEBAR -->
	<!-- start: SETTINGS -->
	<?php include 'include/settings-panel.php'; ?>
	<!-- end: SETTINGS -->
</div>
<!-- start: MAIN JAVASCRIPTS -->
<?php include 'include/main-js.php'; ?>
<!-- start: JavaScript Event Handlers for this page -->
<script src="assets/js/index.js"></script>
<script>
	jQuery(document).ready(function() {
		Main.init();
		Index.init();
	});
</script>
<script>
	var textAreas = document.getElementsByTagName('textarea');

	Array.prototype.forEach.call(textAreas, function(elem) {
		elem.placeholder = elem.placeholder.replace(/\\n/g, '\n');
	});
</script>
<!-- end: JavaScript Event Handlers for this page -->
</body>
</html>